...
// DisableStacktrace disable error stacktrace in log content
func (c *Logger) DisableErrorStacktrace() *Logger {
	logger := c.logger.Desugar().WithOptions(zap.AddStacktrace(zapcore.DPanicLevel)).Sugar()
	return wrap(logger, c.prefix)
}
...
func (c *Logger) Named(name string) *Logger {
	return wrap(c.logger.Named(name), c.prefix)
}
